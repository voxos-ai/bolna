...
class LanceDB(BaseRAG):
    ...
    def __init__(self, embedding_name: BaseEmbed, config: ProviderConfig) -> None:
        ...
        # self.path = f"s3://lancedb-example-data/{self.loc}" : use this to config your lancedb dir path
        
        self.path = os.getenv('LANCEDB_DIR')
        print(f"Initialized LanceDB with path: {self.path}")
        self.verify_shared_directory()

    def verify_shared_directory(self):
    ...
    async def add_index(self, nodes) -> str:
        ...
        print(f"LanceDB Dir Path is : {self.path}")
